...
class QLearning:
    ...
    def fit(self, env, steps=1000, num_bins=100):
        ...
        # loop for steps number of iterations
        for step in range(steps):
            ...
            if src.random.rand() < self.epsilon:
                ...
                action = src.random.choice(actions_with_max_q_value)
            ...
            state_action_values[current_state, action] = state_action_values[current_state, action] + self.alpha * (reward + self.gamma * (np.max(state_action_values[next_state, action])) - state_action_values[current_state, action])

            # update current state
            current_state = next_state

            # append reward to all_rewards
            all_rewards.append(reward)

            # if done, reset environment and update current state
            if done:
                current_state, _ = env.reset()

            # if num_bins steps have been taken, compute and store average reward
            if (step + 1) % (steps // num_bins) == 0:
                avg_reward = np.mean(all_rewards[-steps // num_bins:])
                avg_rewards[(step + 1) // (steps // num_bins) - 1] = avg_reward
    ...
    def predict(self, env, state_action_values):
        ...
        while not done:
            # choose action using the learned state-action values
            q_values = state_action_values[current_state, :]
            max_q_value = np.max(q_values)
            actions_with_max_q_value = np.where(q_values == max_q_value)[0]
            ...
            # append state, action, and reward to lists
