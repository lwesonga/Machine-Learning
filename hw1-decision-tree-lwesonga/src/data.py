...
def train_test_split(features, labels, fraction):
    ...
    if fraction == 1.0:
        ...
        label = int(features.shape[0] * fraction)
        train_features = features[:label]
        test_features = features[label:]
        train_labels = labels[:label]
        test_labels = labels[label:]
...
def cross_validation(features, labels, n_folds):
    ...
    if n_folds == 1:
        ...
        for i in range(n_folds):
            start = i * fold_size
            finish = start + fold_size
            test_features = features[start:finish]
            test_labels = labels[start:finish]
            train_features = np.concatenate((features[:start], features[finish:]), axis=0)
            train_labels = np.concatenate((labels[:start], labels[finish:]), axis=0)
            output.append((train_features, train_labels, test_features, test_labels))
        return output 
